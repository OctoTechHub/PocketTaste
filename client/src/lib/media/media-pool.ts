...
// GET /catalog/{id}/audio (Sarvam TTS, base64 WAV) and wired in on the watch
// page via useAudioClip. A story with no narration simply has no audio — we
// never substitute a placeholder track.
//
// Cover/banner reconstruct the exact picsum URL the upstream `stories` docs
// store (seeded by content_id), so a tile shows the same art the catalog holds.

/** No hardcoded audio — narration comes from the backend or not at all. */
export function mediaFor(_contentId: string): string | undefined {
  return undefined;
}
