...
export interface CommandPaletteProps {
  ...
  /** Optional controlled query, for server-driven search. */
  query?: string;
  onQueryChange?: (query: string) => void;
  /** Rendered above the list — e.g. a loading row. */
  header?: ReactNode;
}
...
export function CommandPalette({
  ...
  const filtered = useMemo(() => {
    // When search is server-driven (query controlled), the caller has already
    // filtered — show everything and let the backend own relevance.
    if (queryControlled) return items;
    if (!query) return items;
    return items.filter((it) => {
      const haystacks = [it.label, it.group ?? "", ...(it.keywords ?? [])];
      return haystacks.some((h) => fuzzyMatch(query, h));
    });
  }, [items, query, queryControlled]);
